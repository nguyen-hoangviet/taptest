...
public class Main {
    public static void main(String[] args) {
            ...
            for (int i = 0; i < subjectNumber; i++) {
                ...
                subjects.add(new Subject(name,null,allMultiplier));
            }
            ...
            for (int i = 0; i < divisionNumber; i++) {
                ...
                String acronym = input.input(3,acronyms,null,"#" +
                        (i + 1) + " division 's name","a valid character");
                acronyms += acronym;
                ArrayList<Subject> divisionSubjects = new ArrayList<>();
                for (int j: divisionSubjectIndexes) {
                    divisionSubjects.add(subjects.get(j));
                }
                double divPassScore = Double.parseDouble(input.input(4,null,null,"#" +
                        (i + 1) + "division pass score","a positive number"));
                ...
                for (int j: divisionSubjectIndexes) {
                    double divMultiplier = Double.parseDouble(input.input(0,null,null,"#"
                            + subjects.get(j).getName() + "'s multiplier in division score","a positive number"));
                    subjects.get(i).setDivisionMultipliers(division,divMultiplier);
                }
            }
            double totalPassScore = Double.parseDouble(input.input(4,null,null,"#" +
               "total pass score","a positive number"));
            String resultInput = input.input(5,acronyms,maxIndexes,"#" +
